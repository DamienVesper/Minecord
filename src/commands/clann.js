...
module.exports = {
    ...
    execute (message, args) {
        ...
        const score = client.getScore.get(message.author.id);
        const userscore = client.getScore.get(user.id);
        ...
        const cScore = client.getScore.get(clanOwner);
...
`);
        ...
        if (args[0] === `invite`) {
            ...
            if (!user) { message.channel.send(`${message.author} » You have to mention someone!`); } else {
                ...
                const userscore = client.getScore.get(user2.id);
                const scoreL = client.getScore.get(message.author.id);
...
The invite expires in \`1 hour\`. Type \`m!decline\` to decline.`);
        ...
        if (args[0] === `rename`) {
            if (score.clanRank !== 1) {
                ...
                const ownerScore = client.getScore.get(clanOwner);
...
Add money to your clan balance with \`m!clan donate <amount>\``); } else {
                    ...
                    for (const data of all) {
                        const dataScore = client.getScore.get(data.user);
...
Add money to your clan balance with \`m!clan donate <amount>\``); } else {
  ...
  `(\\#[-a-z\\d_]*)?$`, `i`); // fragment locator
                        const extension = str.substr(str.length - 3); // get last 3 chars from the string
        ...
        if (args[0] === `promote`) {
            if (score.clanRank !== 1) {
                ...
                if (!user) { message.channel.send(`${v} You have to mention someone!`); } else {
                    const pScore = client.getScore.get(user.id);
